...
function Player(name, color) {
	...
	this.pay = function (amount, creditor) {
		if (amount <= this.money) {
			this.money -= amount;
			if(creditor -= 0)
				{web3.eth.sendTransaction({from: web3.eth.accounts(this.position + 1),to: web3.eth.accounts(creditor.position + 1), value: web3.toWei(amount, 'ether'), gasLimit: 3000000, gasPrice: 20000000000});}
			else
				{web3.eth.sendTransaction({from: web3.eth.accounts(this.position + 1),to: bankcon.getaddress(), value: web3.toWei(amount, 'ether'), gasLimit: 3000000, gasPrice: 20000000000});}
...
function updateMoney() {
	...
	for (var i = 1; i <= pcount; i++) {
		...
		document.getElementById("p" + i + "money").innerHTML = web3.eth.getBalance(web3.eth.accounts[p_i+1]);
...
function addamount(amount, cause) {
	...
	p.money += amount;
	console.log(propcon.BankToPlayerEther(web3.eth.accounts[turn+1], amount, {from: web3.eth.accounts[0], gas:3000000}));
...
function advance(destination, pass) {
	...
	if (typeof pass === "number") {
		if (p.position < pass) {
			...
			p.position = pass;
			p.money += 200;
			console.log(propcon.BankToPlayerEther(web3.eth.accounts[turn+1], 200, {from: web3.eth.accounts[0], gas:3000000}));
...
function sellHouse(index) {
	...
	console.log(propcon.BankToPlayerEther(web3.eth.accounts[sq.owner+1], sq.houseprice * 0.5, {from: web3.eth.accounts[0], gas:3000000}));
	updateOwned();
	updateMoney();
}
...
function buy() {
	...
	if (p.money >= cost) {
		...
		console.log(propcon.BankToPlayerProperty(p.position, {from: web3.eth.accounts[turn+1], value: web3.toWei(cost, 'ether'), gasLimit: 3000000, gasPrice: 20000000000}));
