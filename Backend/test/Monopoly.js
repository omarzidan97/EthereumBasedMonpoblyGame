...
var intialize = artifacts.require("intial");

contract('Game', accounts => {
   it("should setup a game with initial property token", async () => {
     const monopoly = await Property.deployed();
     const monopoly2 = await Bank.deployed();
    // const deployed_game = await intialize.deployed();

     const banker = await monopoly2.BankAddress();
     const firstPropertyOwner = await monopoly.getPropertyOwner.call(1);
